...
export function AIChat(props: AIChatProps) {
  ...
  const [aiChatHistory, setAiChatHistory] =
    useState<RoadmapAIChatHistoryType[]>(defaultMessages ?? []);
  ...
  const { mutate: deleteChatMessage } = useMutation(
    {
      mutationFn: (messages: RoadmapAIChatHistoryType[]) => {
        if (!defaultChatHistoryId) {
          return Promise.resolve({ status: 200, message: 'Chat history not found' });
        }
      ...
      onSuccess: () => {
        ...
        queryClient.invalidateQueries(chatHistoryOptions(defaultChatHistoryId));
      },
      onError: (error) => {
  ...
  const canScrollToBottom = useCallback(() => {
    ...
    const paddingBottom = parseInt(getComputedStyle(scrollableContainer).paddingBottom);
  ...
  const completeAIChat = async (messages: RoadmapAIChatHistoryType[], force: boolean = false) => {
    ...
    await readChatStream(stream, {
      onMessage: async (content) => {
        const jsx = await renderMessage(content, aiChatRenderer, { isLoading: true });
        flushSync(() => setStreamedMessage(jsx));
        setShowScrollToBottomButton(canScrollToBottom());
      },
      onMessageEnd: async (content) => {
        const jsx = await renderMessage(content, aiChatRenderer, { isLoading: false });
        const newMessages: RoadmapAIChatHistoryType[] = [...messages, { role: 'assistant', content, jsx }];
        ...
        queryClient.invalidateQueries({ predicate: (q) => q.queryKey[0] === 'list-chat-history' });
      },
      onDetails: (details) => {
  ...
  useEffect(() => {
    ...
    scrollableContainer.addEventListener('scroll', debouncedHandleScroll, { signal: abortController.signal });
  ...
  const handleRegenerate = useCallback(
    (index: number) => {
      if (isLimitExceeded) {
        if (!isPaidUser) onUpgrade?.();
        toast.error('Limit reached for today. Please wait until tomorrow.');
        return;
      }

      const filteredChatHistory = aiChatHistory.slice(0, index);
      flushSync(() => setAiChatHistory(filteredChatHistory));
      scrollToBottom();
  ...
  const isDataLoading = isLoading || isBillingDetailsLoading || isUserPersonaLoading;
  ...
  return (
    <div className="ai-chat relative flex grow flex-col gap-2 bg-gray-100">
      <div className={cn('scrollbar-none absolute inset-0 overflow-y-auto pb-55', shouldShowUpgradeBanner ? 'pb-60' : 'pb-55')} ref={scrollableContainerRef}>
        <div className="relative mx-auto w-full max-w-3xl grow px-4">
          {shouldShowQuickHelpPrompts && (
            <QuickHelpPrompts onQuestionClick={(question) => { textareaMessageRef.current?.focus(); setMessage(question); }} />
          )}
          {!shouldShowQuickHelpPrompts && (
            <ChatHistory chatHistory={aiChatHistory} isStreamingMessage={isStreamingMessage} streamedMessage={streamedMessage} onDelete={handleDelete} onRegenerate={handleRegenerate} />
          )}
        </div>
      </div>

      {isPersonalizedResponseFormOpen && (
        <PersonalizedResponseForm defaultValues={userPersona?.chatPreferences ?? undefined} onClose={() => setIsPersonalizedResponseFormOpen(false)} />
      )}

      <div className="pointer-events-none absolute right-0 bottom-0 left-0 mx-auto w-full max-w-3xl px-4" ref={chatContainerRef}>
        <div className="mb-2 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <QuickActionButton icon={PersonStandingIcon} label="Personalize" onClick={() => {
          ...
          <div className="flex items-center gap-2">
            {showScrollToBottomButton && <QuickActionButton icon={ArrowDownIcon} label="Scroll to Bottom" onClick={scrollToBottom} />}
            {aiChatHistory.length > 0 && !isPaidUser && <QuickActionButton icon={TrashIcon} label="Clear Chat" onClick={() => { setAiChatHistory([]); deleteChatMessage([]); }} />}
          </div>
        </div>

        <form className="pointer-events-auto relative flex flex-col gap-2 overflow-hidden rounded-lg rounded-b-none border border-b-0 border-gray-200 bg-white p-2.5" onSubmit={(e) => { e.preventDefault(); if (isDataLoading) return; handleChatSubmit(); }}>
          <AutogrowTextarea ref={textareaMessageRef} value={message} onChange={(e) => setMessage(e.target.value)} className="min-h-10 w-full resize-none bg-transparent text-sm focus:outline-none" placeholder="Ask me anything..." disabled={isStreamingMessage} autoFocus onKeyDown={(e) => {
          ...
          {isLimitExceeded && isLoggedIn() && !isDataLoading && (
            <div className="absolute inset-0 z-10 flex items-center justify-center gap-2 bg-black text-white">
              ...
              <p className="cursor-not-allowed">Limit reached for today{isPaidUser ? '. Please wait until tomorrow.' : ''}</p>
              {!isPaidUser && <button type="button" onClick={() => { onUpgrade?.(); }} className="rounded-md bg-white px-2 py-1 text-xs font-medium text-black hover:bg-gray-300">Upgrade for more</button>}
            </div>
          )}

          <div className="flex justify-end">
            <button type="submit" className="flex size-8 shrink-0 items-center justify-center rounded-md border border-gray-200 disabled:cursor-not-allowed disabled:opacity-50" disabled={isLimitExceeded || isStreamingMessage || isDataLoading}>
