...
// ─── TDD Project Configs ──────────────────────────────────────────────────────
// These drive the strict TDD loop in ProjectLearningPage. Any project whose
// slug appears here gets the full "Run & Test" validation flow instead of the
// passive tutorial view.

import type { ProjectConfig } from '@/types/project'

export const tddProjectConfigs: Record<string, ProjectConfig> = {
  'ecommerce-auditor': {
    slug: 'ecommerce-auditor',
    title: 'E-commerce Auditor',
    language: 'python',
    roleId: 'data-analyst',
    steps: [
      {
        id: 'step-1',
        title: 'Step 1: Calculate Total Revenue',
        callableName: 'calculate_revenue',
        instructions: `## Calculate Total Revenue

You're given a list of sales records. Each record is a Python dictionary with:

- \`user_id\` — unique customer ID  
- \`revenue\` — amount spent (int/float)  
- \`churned\` — whether the customer has left (bool)

**Your task:** Write a function \`calculate_revenue(records)\` that sums all \`revenue\` values and returns the **total as an integer**.

> 💡 Loop through the list and accumulate each record's \`"revenue"\` key.`,
        initialCode: `data = [
    {"user_id": 1, "revenue": 1200, "churned": False},
    {"user_id": 2, "revenue": 450,  "churned": True},
    {"user_id": 3, "revenue": 1000, "churned": False},
]

def calculate_revenue(records):
    # TODO: sum all revenue values and return the total
    pass
...
print(calculate_revenue(data))`,
        testCases: [
          {
            hidden: false,
            expected_output: '2650',
          },
          {
            hidden: true,
            input_data: [
              { user_id: 10, revenue: 300, churned: false },
              { user_id: 11, revenue: 500, churned: true },
            ],
            expected_output: '800',
          },
        ],
      ...
      {
        id: 'step-2',
        title: 'Step 2: Filter Churned Users',
        callableName: 'filter_churned',
        instructions: `## Filter Churned Users

Using the same sales records, identify which customers have left the platform.

**Your task:** Write a function \`filter_churned(records)\` that returns a **list** containing only the records where \`churned\` is \`True\`.

The hidden test will check the **length** of the returned list using \`len()\`.

> 💡 A list comprehension like \`[r for r in records if r["churned"]]\` works perfectly here.`,
        initialCode: `data = [
    {"user_id": 1, "revenue": 1200, "churned": False},
    {"user_id": 2, "revenue": 450,  "churned": True},
    {"user_id": 3, "revenue": 1000, "churned": False},
...
def filter_churned(records):
    # TODO: return only records where churned is True
    pass

print(len(filter_churned(data)))`,
        testCases: [
          {
            hidden: false,
            expected_output: '1',
          },
          {
            hidden: true,
            input_data: [
              { user_id: 10, revenue: 300, churned: true },
              { user_id: 11, revenue: 500, churned: true },
              { user_id: 12, revenue: 200, churned: false },
            ],
            expected_output: '2',
          },
        ],
