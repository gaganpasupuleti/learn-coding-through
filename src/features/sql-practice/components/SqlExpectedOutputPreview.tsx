...
export function SqlExpectedOutputPreviewPanel({ question, preview }: SqlExpectedOutputPreviewPanelProps) {
  if (preview.status === 'loading') {
    return (
      <div className={cn('flex items-center gap-2 p-4 text-sm', wb.textMuted)}>
  ...
  if (preview.status === 'error') {
    return (
      <div className={cn('space-y-2 p-4 text-sm', wb.textMuted)}>
        <p className="text-rose-200">{preview.errorMessage ?? 'Unable to load expected output preview.'}</p>
        <p className="text-xs">Expected columns from question: {question.expectedColumns.join(', ') || '—'}</p>
      </div>
    )
  }
  ...
  if (preview.status === 'idle') {
    return (
      <div className={cn('p-4 text-sm', wb.textMuted)}>
        Select a checkable question to preview expected output shape.
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      <p className={cn('text-xs', wb.textMuted)}>
        Preview shows result shape only. Solution SQL is not shown here.
      </p>
    </div>
  )
}
